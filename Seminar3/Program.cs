...
namespace Seminar3
{
    internal class Program
    {
        ...
        {
            Comanda c1 = new Comanda("Adnana Orascu", "adnanaorascu@email.com");
            c1.AdaugaProdus(new Produs("mouse", 70m, 1));
            c1.AdaugaProdus(new Produs("tastatura", 120.99m, 1));

            Depozit d = new Depozit("Stefanesti");

            Console.WriteLine("Introdu canalele de notificare (ex: sms email log):");
            var optiuni = Console.ReadLine();
            NotificareClient notificator = BuildNotificator(optiuni);
            d.InregistreazaComanda(c1, notificator);

            d.ComandaSchimbatStare += (object sender, ComandaSchimbatStareEventArgs e) =>
            {
                Console.WriteLine($"Comanda {e.Comanda.NumarComanda} din {e.StareVeche} in {e.StareNoua}");
            };
        ...
        {
            Console.WriteLine($"Comanda {e.Comanda.NumarComanda} a fost livrata la data de {e.DataLivrare:dd.MM.yyyy}");
        }
